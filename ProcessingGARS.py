...
def study_subject(label, red, green, subject,pixdim=None):
    ...
    for l in range(1, np.max(label)+1):
        print("Taking care of %d out of %d" % (l, np.max(label)+1))
...
def process_subject(path_jpeg, path_save, subject_name, thresh=64,
                    pixdim=None, not_stop=True):
    '''
...
def main(argv):
    ...
    parser.add_argument('-dz', type=float, dest='pixdim_z', help='pixel ' \
                                                              'dimension out of '
                                                     'plane', default=1.750)
    parser.add_argument('-stop', dest='stop_early', help='stop '
                                                                     'process ' \
                                                                 'dimension out of '
                                                                 'plane',
                        action='store_true'
        ...
        if s not in FORBIDDEN:
            ...
            if args.stop_early:
                ...
                if not os.path.exists(os.path.join(dirname, s,
                                  'ExtractedTableFinFilled2_'+s+'.csv')):
                    try:
